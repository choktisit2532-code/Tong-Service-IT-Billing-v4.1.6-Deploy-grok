...
import {
  ...
  openDocumentModal,
  closeDocumentModal,
  initDocumentWizardEventListeners
} from './document-wizard.js';

initTheme();

if (!getToken()) location.replace('./index.html');

// DOM helpers (kept local for performance in hot paths)
...
// === Document Wizard (moved to ./document-wizard.js) ===
...
// Note: Full openDocumentModal implementation remains in app.js for stability during Phase 1.
// In future phases we can fully migrate the body to document-wizard.js
...
function renderAdvancedReport(data) {
  ...
  createChart('advanced-revenue-chart', {
    type: 'bar',
    data: { labels: revenueLabels, datasets: [{ label: 'ยอดรับสุทธิ', data: revenueValues, borderRadius: 8 }] },
    options: baseChartOptions({ showLegend: false })
  ...
  createChart('advanced-sales-type-chart', {
    type: 'doughnut',
    data: { labels: salesLabels, datasets: [{ data: salesValues, borderColor: theme.surface, borderWidth: 2 }] },
    options: baseChartOptions({ moneyAxis: false })
  ...
  createChart('advanced-aging-chart', {
    type: 'bar',
    data: { labels: agingLabels, datasets: [{ label: 'ยอดค้างรับ', data: agingValues, borderRadius: 8 }] },
    options: baseChartOptions({ showLegend: false })
  }, agingValues.some((value) => value > 0));
  refreshIcons();
}
...
function renderAuditLogs(rows, meta) {
  ...
  $('#audit-table').innerHTML = rows.length ? rows.map((row) => `
    <tr>
      ...
      <td><span class="status-badge status-info">${escapeHtml(row.action)}</span></td>
      <td>${escapeHtml(row.entity_type)}</td>
      <td>${escapeHtml(row.entity_id || '-')}</td>
      <td><code class="json-preview">${escapeHtml(JSON.stringify(row.details || {}))}</code></td>
...
$('#load-audit')?.addEventListener('click', () => loadAuditLogs().catch(showGlobalError));
...
}, true); // Use capture phase for better backdrop click detection

// Close sidebar on Escape key (Accessibility)
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && sidebar.classList.contains('open') && window.innerWidth <= 820) {
    closeSidebar();
  }
});
